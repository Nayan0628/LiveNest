...
export async function updateRecording(recordingId, updateData) {
  const response = await axiosInstance.put(`/recordings/${recordingId}`, updateData);
  return response.data;
}
...
export async function getRecordingById(recordingId) {
  const response = await axiosInstance.get(`/recordings/${recordingId}`);
  return response.data;
}
...
export async function fetchRecordingUrl(recordingId) {
  try {
    const response = await axiosInstance.post(`/recordings/fetch-url/${recordingId}`);
    return response.data;
  } catch (error) {
    console.error("Error in fetchRecordingUrl:", error);
    throw error;
  }
}
